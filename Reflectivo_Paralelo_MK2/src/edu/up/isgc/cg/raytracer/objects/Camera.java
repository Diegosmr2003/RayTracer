 ...
 *  Date: 2024-06-06
 *  Description:
 *  This class represents a camera in a ray tracing environment. The camera is used to capture the scene by projecting
 *  rays from its position through each pixel in its field of view (FOV). The camera's resolution determines the number
 *  of rays cast, and the near and far planes define the depth range of objects visible to the camera. The camera's
 *  position and orientation are defined by a Vector3D.
 */
